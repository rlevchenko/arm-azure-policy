...
$netbios = "dom-" + $args[0]
$gw = "10.10.$($args[0]).1" #define yours or retrieve them by using args
$ip = "10.10.$($args[0]).20" #ip for this EXCH node
$dns1 = "10.10.$($args[0]).10" # 1 dns server IP 
$dns2 = "10.10.$($args[0]).11" # 2 dns server IP
$dagip = "10.10.$($args[0]).23" # vIP for DAG
...

{
    ...
    Node $AllNodes.Where{ $_.Role -eq 'FirstDAGMember' }.NodeName
    {
        ...
        ### 05/10/2016 MailPolicy script is temporarily disabled ###
